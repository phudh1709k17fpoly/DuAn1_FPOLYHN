...
if(!isset($error)){
  $name = $ho."-".$ten;
   $sql = "UPDATE `addres_user` SET `name`='$name',`phone`='$phone',`adress`='$address',`note`='$note' WHERE `id`=$_GET[id]";
...
$public = $class->updateDate($_GET['id']);
...
<form action="" method="post" id="form-cart">
    <div class="col">
      <input type="hidden" value="<?=$public['name']?>" name="name">
  ...
  <label class="form-label" for="form3Example3">Số điện thoại</label>
    <input type="number" id="phone" class="form-control" name="phone" value="<?=$public['phone']?>" />
  ...
  <label class="form-label" for="form3Example4">Địa chỉ</label>
    <input type="text" id="address" class="form-control" name="address" value="<?=$public['adress']?>" />
  ...
  <label class="form-label" for="form4Example3">Chi tiết địa chỉ</label>
    <textarea class="form-control" id="form4Example3" rows="4" name="note" placeholder="số nhà/đườnng/xã,quận,phường/"><?=$public['note']?></textarea>
